...
angular.module(ApplicationConfiguration.applicationModuleName).controller('LanguageController' , ["$scope", function($scope){
	$scope.language = 'it';
...
// Use applicaion configuration module to register a new module
ApplicationConfiguration.registerModule('works');
'use strict';
...
.directive('addPhoto', ["$compile", function($compile){
	return{
		restrict: 'A',
		link: function(scope , elem, attrs , ctrl){
			scope.counter = 0;
...
.directive('single', ["Works", function(Works){
	return{
		restrict: 'A',
		controller: ["$scope", "Works", function($scope , Works) {
